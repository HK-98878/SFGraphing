...
SFPieChart::SFPieChart(const PieChartDataSet& dataset, const sf::Vector2f& position,
                       const float& radius, const sf::Font& font)
...
void SFPieChart::GenerateVertices() {
  ...
  legendBasePosition.y -= (int)(_dataset.GetLength() / 2) * charSize;
  ...
  for (size_t i = 0; i < _dataset.GetLength(); i++) {
    ...
    // Middle point
    segment.append({_position, _dataset.GetColor(i)});

    float segmentAngle = (_dataset.GetValue(i) / totalSegmentsValue) * static_cast<float>(2 * M_PI);

    for (float theta = segmentBaseAngle; theta < segmentBaseAngle + segmentAngle + angleStep;
         theta += angleStep) {
      float xPos = _position.x + _radius * cos(theta);
      float yPos = _position.y + _radius * sin(theta);

      segment.append({sf::Vector2f(xPos, yPos), _dataset.GetColor(i)});
    }
