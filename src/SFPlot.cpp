...
sf::Vector2f SFPlot::CoordToWindowPosition(const sf::Vector2f& coords) {
  ...
  windowPosition.x =
    _origin.x + (_margin + xAxisLength * ((coords.x - _xCoordBounds.x) / xAxisRange));
  windowPosition.y = _origin.y + (_dimension.y - _margin -
                                  yAxisLength * ((coords.y - _yCoordBounds.x) / yAxisRange));
...
SFPlot::SFPlot(const sf::Vector2f& position, const sf::Vector2f& dimension, const float& margin,
               const sf::Font& font, const std::string& xLabel, const std::string& yLabel)
    : _origin(position),
...
void SFPlot::SetupAxes(const float& xmin, const float& xmax, const float& ymin, const float& ymax,
                       const float& xstep, const float& ystep, const sf::Color& axesColor) {
  _xCoordBounds.x = xmin;
  _xCoordBounds.y = xmax;
...
void SFPlot::AddDataSet(const PlotDataSet& data_set) { _plotDataSets.push_back(data_set); }
